...
# It detects if nerdctl.lima is available to build in an isolated environment. Otherwise, it falls back to a standard local npm build.
...
if command -v nerdctl.lima &>/dev/null; then
	...
	echo "Step 1: ⚠️ nerdctl.lima not found. Falling back to local npm build..."

	if command -v npm &>/dev/null; then
		echo "📦 Running 'npm run build' locally..."
		npm run build
	else
		echo "❌ Error: Neither nerdctl.lima nor npm is installed. Cannot build."
		exit 1
	fi
fi
