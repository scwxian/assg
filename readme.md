...
* **Lazy-Loaded "Hydration"**: Interactive components—such as the Instagram feed or About page stat bars—are stored in `js/modules`. Using an `IntersectionObserver` in `main.js`, these modules are strictly **lazy-loaded**; the JavaScript is fetched and executed only when the user scrolls the component into view, keeping the initial page load extremely lightweight.
